class Videatra
  Sinatra::Videoman::Manager.config do |config|
    ...
    config[:video_upload_dir] = settings.root + settings.upload_spec[:videos][:upload_dir]
    ...
    config[:thumb_upload_dir] = settings.root + settings.upload_spec[:thubmnails][:upload_dir]
